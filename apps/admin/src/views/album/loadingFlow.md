...
宫格：扁平时间线无目录树（`groups` 全部 files flatMap）；**排序/筛选只在前端** `filteredFiles`（`capture_at` 升序旧→新，同秒/无拍摄时间再比文件名；**不用** `modified`；无拍摄时间沉底）；Rust discover/DB 不排拍摄序；扫描/筛选后滚到底部；工具栏含筛选统计与宫格/列表切换（列表 vxe 虚拟滚动）；文件名模糊 + 拍摄日区间；宫格左下角浮层为可视区首～末张 `YYYY年MM月DD日`。
