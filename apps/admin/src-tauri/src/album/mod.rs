...
/**
 * 同步落盘改名后：搬迁 media.db 行与 thumb/preview/playback 缓存，避免宫格丢缩略图
 * @note 无相册索引行时静默成功；失败只打日志，不回滚源文件重命名
 */
pub fn relocate_media_after_rename(
  app: &AppHandle,
  old_path: &str,
  new_path: &str,
) -> Result<(), String> {
  ...
  let thumb_size = settings.thumb_size.max(1);
  let conn = db::open_db(&album_data_dir)?;

  let has_row: bool = conn
    .query_row(
      "SELECT 1 FROM media WHERE path = ?1",
      rusqlite::params![old_path],
      |_| Ok(true),
    )
    .optional()
    .map_err(|e| format!("探测 media 行失败: {e}"))?
    .unwrap_or(false);

  if !has_row {
    // 仍可能有其它行的 video_path 指向旧 mov
    let _ = conn.execute(
  ...
  let (thumb, preview, _video, playback) = db::get_media_companion_paths(&conn, old_path)?;

  let new_meta = std::fs::metadata(new_path).map_err(|e| format!("读取新文件元数据失败: {e}"))?;
  ...
  let (new_thumb, new_preview, new_playback) = thumbnail::relocate_derived_caches(
    &cache_dir,
    old_path,
    new_path,
    new_modified,
  ...
  let _ = db::relocate_media_path(
    ...
    new_modified,
    new_thumb.as_deref(),
    new_preview.as_deref(),
    new_playback.as_deref(),
  )?;
  Ok(())
}
